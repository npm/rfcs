...
withdrawn_at: null
implementation: https://github.com/npm/cli/pull/9439
---
...
3. Determine the version the patch was authored against — parsed from the patch filename (`<name>@<version>.patch`) — and extract both that baseline tarball and the new version's tarball into temporary directories.
4. Rebase the patch onto the new version with a 3-way merge, computed in a throwaway git repository: commit the baseline, apply the existing patch onto it (using the same strict, fuzz-0 apply the installer uses) as one branch, commit the new version as the other branch, and `git merge` the two. (`git apply --3way` is **not** used — npm's patches are jsdiff unified diffs without git blob-index lines, so they cannot drive git's native 3-way fallback. If the existing patch no longer applies cleanly to its own baseline, `update` errors and asks the user to rebase by hand.)

**Clean apply.**

5. Re-diff the merged result against the pristine new tarball; write the result to a new patch file under the new selector (`<patches-dir>/<name>@<new-version>.patch` by default).
...
8. Leave the edit directory on disk with `<<<<<<<` / `>>>>>>>` conflict markers (written by `git merge`), print the path, and instruct the user to resolve and re-run `npm patch commit <edit-dir>` — no new "resolve" command. When `commit` is finishing an `update` (detected via a small marker file the conflict path leaves in the edit directory), it keeps the operation metadata-only — manifest + lockfile, no `node_modules` writes — to match `update`'s own semantics, and for an exact rename it also drops the renamed-from selector. The marker is only honored when it names a real, same-package selector, so it cannot hijack an ordinary commit.

**Ordering and recovery.** `update` performs everything that can fail — extracting the tarballs, the 3-way merge, computing the diff — *before* it mutates the project. It then writes in a fixed order: the new patch file, then `package.json`, then the lockfile (through a `--package-lock-only` reify that records the new `patched` integrity without touching `node_modules`). The old patch file is removed only *after* the lockfile write succeeds, so a failure never deletes the old patch before the new one is recorded, and no selector is ever left pointing at a missing file. The throwaway rebase work tree is removed on every path, success or failure (on conflict the resolved edit directory is intentionally left for the user). `update` does not implement a full fsync/`rename(2)` transaction: if the process is killed between the `package.json` write and the lockfile write, the manifest references the new selector while the lockfile is momentarily stale — a state the next `npm install` / `npm ci` reconciles.
...
This bumps `lockfileVersion` from `3` to `4`. Feature-aware clients recognize v4 and the per-package `patched` records and apply patches during reify; the parse path also gains a `lockfileVersion > maxLockfileVersion` guard ([`workspaces/arborist/lib/shrinkwrap.js`](https://github.com/npm/cli/blob/latest/workspaces/arborist/lib/shrinkwrap.js)) so a feature-aware client refuses a still-newer lockfile (v5+) rather than guessing.

The bump is **not** a fail-closed gate for already-released npm. npm 7+ reads `data.lockfileVersion` as-is — there is no version ceiling in released `shrinkwrap.js` — and silently ignores unknown per-package fields like `patched`. So a pre-feature client reading a v4 lockfile does **not** abort; it installs the resolved versions **unpatched** (fail open). The bump is a format marker and a forward guard for feature-aware clients; there is no way to retroactively make an older npm error on a patched project.

A project pinning a lower `lockfile-version` (e.g. 3) still cannot honor it when patches are present, because the `patched` records and the forward guard require v4; npm writes version 4 and warns that the lockfile was upgraded.

> Note: a hard floor for older clients is available, opt-in, via `engines`. `patchedDependencies` is honored only in a root manifest, so the unpatched-install risk applies to installs *of the patched project itself* (its developers and CI), not to consumers of a published package (which never apply its patches). A project that wants those installs to hard-fail on a too-old npm can declare an `engines.npm` floor and commit `.npmrc` with `engine-strict=true`: npm's existing engine check (`npm-install-checks`'s `checkEngine`, run over the ideal tree in [`workspaces/arborist/lib/arborist/build-ideal-tree.js`](https://github.com/npm/cli/blob/bf623e0a9ea568a47b777c563e48a097cb12e442/workspaces/arborist/lib/arborist/build-ideal-tree.js)) throws `EBADENGINE` and aborts when `engine-strict` is set. This is pre-existing behavior, so it works on already-released npm. The caveats are that `engine-strict` is opt-in (it defaults to warn-only) and that a root declaring `devEngines` bypasses this `checkEngine` path.
...
| Lockfile version | bumped to `4`; pre-feature clients install **unpatched** (fail open) — a hard floor is opt-in via `engines.npm` + `engine-strict` |
...
- Lockfile migration: v3 → v4 bump; a pre-feature client installs a v4-with-patches lockfile **unpatched** (documented fail-open, not an abort); a feature-aware client refuses a v5+ lockfile with `ELOCKFILEVERSION`; `engines.npm` + `engine-strict=true` aborts a too-old client with `EBADENGINE`.
...
- Lockfile schema: `patched.{path,integrity}` per node, `lockfileVersion: 4`, with `npm ci` enforcing hash match; pre-feature clients install v4 lockfiles **unpatched** (fail open) and a hard floor is opt-in via `engines.npm` + `engine-strict`.
